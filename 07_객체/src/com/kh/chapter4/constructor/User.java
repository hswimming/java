 ...
 * 	- 생성자 작성 시 주의사항
 * 	  1. 생성자의 이름은 반드시 클래스 이름과 동일해야 한다.
 * 	  2. 반환형이 존재하지 않는다.
 * 	  3. 매개 변수가 있는 생성자를 작성하게 되면 기본 생성자를 자동으로 만들어주지 않는다.
 */
...
public class User {
	...
	// source 메뉴에서 generate constructor using field
...
//		this.name = name;
		...
		// 1. 위 경우 String 형태의 요소들이 위에 이미 생성되어 있기 때문에 묶어서 사용, 사용하려면 반드시 생성해놓아야함
		// 2. 반드시 맨 위에서 사용
		// 3. 생성자 호출에서만 사용 가능, 일반 메소드에서 사용 불가능
		this.age = age;
		this.gender = gender;
	}
