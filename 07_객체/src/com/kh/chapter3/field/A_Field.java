 ...
 * 
 * 2. 지역 변수
 * 	- 클래스 영역 안에 특정한 구역({})에 선언하는 변수, 해당 구역에서만 사용이 가능하다. (메소드, 제어문(if, for) 등) 
 * 	- 지역 변수는 접근 제한자를 사용할 수 없다.
 * 	- 지역 변수의 생성과 소멸
 * 	  생성 시점 : 특정한 구역({}) 실행 시 메모리(Stack)에 할당된다.
 * 	  소멸 시점 : 특정한 구역({}) 실행 종료 시 소멸된다. (ex. 메소드 종료 시)
 */
...
public class A_Field {
	...
	// 매개 변수가 있는 메소드
	// 매개 변수도 일종의 지역 변수로 메소드 영역에서 사용이 가능하다. (메소드가 끝나면 소멸)
	public void test(int number) { //매개 변수에는 접근 제한자를 붙이지 않는다.
