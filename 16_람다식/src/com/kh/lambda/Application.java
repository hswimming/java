...
public class Application {
	...
	public static void main(String[] args) {
		 ...
		 * 			- (a) -> { System.out.println(a); }; // 매개변수가 하나일 때 자료형 생략 가능
...
//		new A_Lambda().method1();
		...
		/*
		 * 타겟타입(Target Type)
		 * 	- 람다식은 단순히 메소드를 선언하는 것이 아니라 람다식으로 선언 된 메소드를 가지고 있는 인스턴스를 생성한다.
		 * 	- 람다식은 인터페이스 타입의 변수에 대입되고 람다식은 인터페이스의 익명 구현 객체를 생성한다.
		 * 	- 람다식은 대입 될 인터페이스의 종류에 따라서 작성 방법이 달라지기 때문에 람다식이 대입 될 인터페이스를 타켓 타입이라고 한다.
		 * 
		 * 함수적 인터페이스(Functional Interface)
		 *	- 모든 인터페이스가 타겟 타입으로 사용할 수 없다.
		 *	- 하나의 추상 메소드가 선언 된 인터페이스만 람다식의 타겟이 될 수 있는데 이러한 인터페이스를 함수적 인터페이스라고 한다.
...
//		new B_Functional().method4(100);
		...
		/*
		 * 표준 함수적 인터페이스
		 * 	- 자바 8부터 빈번히 사용되는 함수적 인터페이스는 java.util.function 패키지에 API로 제공된다.
