...
public class A_File {
	...
	public void method1() {
		...
		try {
			...
			System.out.println("절대 경로 : " + file.getAbsolutePath()); // 전체 경로
			System.out.println("상대 경로 : " + file.getPath()); // 현재 프로젝트 기준 어디 있는지
			System.out.println("파일 용량 : " + file.length());
			System.out.println("파일 존재 여부 : " + file.exists());
			
			file.createNewFile();
			
			// 존재하는 경로까지 지정 해주면 해당 위치에 파일을 생성한다.
			// 만약에 존재하지 않는 경로를 제시하면 IOExceptiond 발생한다.
			File file2 = new File("D:/test2.txt");
//			File file2 = new File("D:/temp/test2.txt");
			
//			file2.mkdirs(); // 디렉토리를 만드는 명령어
			file2.createNewFile();
