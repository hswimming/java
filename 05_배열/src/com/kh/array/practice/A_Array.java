...
//		iArray1 = new int[]; // 배열의 크기를 지정하지 않으면 에러가 발생한다.
		...
		/*
		 * 배열을 생성하고 초기화 해주지 않아도 문제 없이 출력이 된다.
		 * 배열은 따로 초기화 하지 않는다면, JVM 이 지정한 기본 값으로 배열이 초기화 된다.
...
//		iArray2.length = 10; // 파이널 변수라서 변경 불가능
...
//		for (int i = 0; i <= iArray.length; i++) {
		 ...
		 * (메소드를 재정의 하지 않으면 기본적으로 객체의 주소값을((10진수) 리턴한다.)
		 */
...
//		iArray2 = {8, 7, 6, 5, 4, 3, 2};
		...
		// Arrays.toString 메소드 -> 요소들을 구분, 정렬해서 출력해줌
		
		String[] fruitArray = new String[5];
		Scanner scanner = new Scanner(System.in);
		
//		System.out.println(Arrays.toString(fruitArray)); // null 값 보기 위해 썼었음
