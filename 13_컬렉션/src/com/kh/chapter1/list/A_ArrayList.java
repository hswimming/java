...
public class A_ArrayList {
	...
	public void method1() {
		...
		String str = (String) list.get(0); // 다운 캐스팅 (object는 최상위 클래스이기 때문)
...
//		System.out.println(now);
		...
		*/
		...
		// 원하는 인덱스 위치에 인스턴스를 추가할 수 있다. (추가되면 한칸씩 밀려난다.)
		list.add(2, 125000);
...
//		list.remove(3.14);
		...
		// 저장된 인스턴스를 모두 삭제할때는 clear() 메소드를 사용한다.
		list.clear();
		System.out.println(list);
		System.out.println(list.isEmpty());
	}
	...
	public void method2() {
		...
		ArrayList<Integer> list2 = new ArrayList<Integer>(); // 기본 타입으로 지정 불가 (ex. int)
...
//		String banana = list.get(0); // list가 반환하는 타입이 String이라서 형변환 생략 가능
//		String apple = list.get(1);
		
		list2.add(7); // Integer로 오토박싱
		list2.add(1);
		list2.add(8);
		list2.add(10);
		list2.add(3);
		...
		// 내림차순으로 리스트 정렬 (오름차순으로 정렬해준 후 실행)
	...
	public void method3() {
		...
		list.add(new Music("시간의 지평선", "윤하", 1));
...
//		});
		...
		// 람다식을 사용하는 방법
		Collections.sort(list, (o1, o2) -> o1.getTittle().compareTo(o2.getTittle()));
