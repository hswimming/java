...
//		th1.setPriority(10);
		th1.setPriority(Thread.MAX_PRIORITY); // 우선순위 -> start 호출하기 전
//		th1.run();
		th1.start();
		
		System.out.println("th1.getPriority() : " + th1.getPriority()); // 우선순위를 보여주는 메소드
...
//		th2.run();
		 ...
		 * 		- 중괄호 ({...})에는 인터페이스에 선언된 모든 추상 메소드들을 재정의 해야 한다.
		 */	 
		 ...
		 */	
		Thread th4 = new Thread(() -> { // 람다식의 -> 뒤 () 매개변수 , {} 실행하려는 코드
			// 작업하고자 하는 코드를 작성한다.
			for (int i = 1; i <= 100; i++) { 
				System.out.println(Thread.currentThread().getName() + "[" + i + "]");
			}
		...
		try {
			/*
			 * join() 메소드를 호출한 스레드(main 스레드)는 아래의 스레드가 종료될 때까지 기다렸다가 아래의 스레드가 종료되면
			 * join() 메소드를 호출한 스레드(main 스레드)를 다시 실행한다.
			 */
			th1.join();
			th2.join();
			th3.join();	
